...
export default function IngresoDetallePage() {
    ...
    const fetchIngreso = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/Ingreso/${params.id}`,
          {
            headers: {
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    if (!res.ok) {
      ...
      setFormError("Error al actualizar el ingreso");
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Detalle del Ingreso</h1>
      ...
      <div className="border rounded-xl p-5 shadow-sm space-y-3">
        <div>
          ...
          <p className="font-semibold text-lg">{ingreso.concepto}</p>
        </div>
        ...
        <div>
          ...
          <p>{new Date(ingreso.fecha).toLocaleDateString()}</p>
        </div>
        ...
        {ingreso.notaAdicional && (
          <div>
            <span className="text-sm text-muted-foreground">
              Nota adicional
            </span>
            <p className="italic">{ingreso.notaAdicional}</p>
          </div>
        )}
      </div>
            ...
            <div className="grid gap-4 py-4 overflow-y-auto overflow-x-hidden">
              ...
              <div className="grid gap-1">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="general">Ingreso general</SelectItem>
                    <SelectItem value="parcela">Ingreso de parcela</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              {!formData.esGeneral && (
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              el ingreso de {ingreso.concepto} por $
              {ingreso.monto.toLocaleString()}.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
